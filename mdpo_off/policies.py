...
from stable_baselines.mdpo_off.tf_tsallis_statistics import *
...
def apply_squashing_func(mu_, pi_, logp_pi, action=None):
    """
    ...
    if action is not None:
        logp_pi -= tf.reduce_sum(tf.log(clip_but_pass_gradient(1 - action ** 2, lower=0, upper=1) + EPS), axis=1, keepdims=True)
        return deterministic_policy, action, logp_pi
    else:
        # OpenAI Variation:
        # To avoid evil machine precision error, strictly clip 1-pi**2 to [0,1] range.
        logp_pi -= tf.reduce_sum(tf.log(clip_but_pass_gradient(1 - policy ** 2, lower=0, upper=1) + EPS), axis=1, keepdims=True)
...
class SACPolicy(BasePolicy):
    ...
    def make_actor(self, obs=None, action=None, reuse=False, scope="pi"):
        """
...
class FeedForwardPolicy(SACPolicy):
    ...
    def make_actor(self, obs=None, action=None, action_unsquashed=None, reuse=False, scope="pi", log_type="q-log", q_prime=1, reparameterize=True):
        if obs is None:
        ...
        if log_type=="q-log":
            if action is None:
                ...
                squashed_mu = tf.tanh(mu_)
                squashed_pi = tf.tanh(pi_)
                logp_pi = -0.5 * (((pi_-mu_)/(tf.exp(log_std)+EPS))**2 + 2*log_std + np.log(2*np.pi)) - tf.log(clip_but_pass_gradient(1 - squashed_pi**2, lower=0, upper=1)+1e-6)
            else:
                pi_ = action_unsquashed
                squashed_mu = tf.tanh(mu_)
                squashed_pi = action
                logp_pi = -0.5 * (((action_unsquashed-mu_)/(tf.exp(log_std)+EPS))**2 + 2*log_std + np.log(2*np.pi)) - tf.log(clip_but_pass_gradient(1 - action**2, lower=0, upper=1)+1e-6)
            
            q_logp_pi = tf.reduce_sum(tf_log_q(tf.exp(logp_pi),q),axis=1) #tf.cond(tf.constant(q > 0., dtype=tf.bool), true_fn=lambda: tf.reduce_sum(tf_log_q(tf.exp(logp_pi),q),axis=1), false_fn=lambda: tf.reduce_sum(tf_log_q(tf.minimum(tf.exp(logp_pi),tf.pow(10.,8/(1-q))),q), axis=1))
            ...
            if action is None:
                ...
                q_logp_pi = gaussian_likelihood(pi_, mu_, log_std)
                # Apply squashing and account for it in the probabilty
                squashed_mu, squashed_pi, q_logp_pi = apply_squashing_func(mu_, pi_, q_logp_pi)
            else:
                pi_ = action_unsquashed
                # Reparameterization trick
                q_logp_pi = gaussian_likelihood(action_unsquashed, mu_, log_std)
                # Apply squashing and account for it in the probabilty
                squashed_mu, squashed_pi, q_logp_pi = apply_squashing_func(mu_, action_unsquashed, q_logp_pi, action)
            
            #q_logp_pi = tf.reduce_sum(q_logp_pi, axis=1)     

        self.policy = squashed_pi
        self.deterministic_policy = squashed_mu
        self.entropy = gaussian_entropy(log_std)
        # MISSING: reg params for log and mu

        return squashed_mu, squashed_pi, q_logp_pi, pi_
