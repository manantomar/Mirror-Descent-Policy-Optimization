...
def traj_segment_generator(policy, env, horizon, reward_giver=None, gail=False, entcoeff=0.0):
    """
    ...
    :param gail: (bool) Whether we are using this generator for standard mdpo or with gail
    ...
    while True:
        action, vpred, states, neglogp = policy.step(observation.reshape(-1, *observation.shape), states, done)
        ...
        if gail:
            ...
            true_reward = reward
        rewards[i] = reward - entcoeff * neglogp
        true_rewards[i] = true_reward - entcoeff * neglogp
        dones[i] = done
        episode_start = done
...
def add_vtarg_and_adv(seg, gamma, lam):
    ...
    ############### NEW ADDITION BELOW #########################
    #gaelam_phi = np.empty(rew_len, 'float32')
    #lastgaelam = 0
    #for step in reversed(range(rew_len)):
    #    nonterminal = 1 - float(episode_starts[step + 1])
    #    delta = rewards[step] + gamma * vpred[step + 1] * nonterminal - vpred[step]
    #    gaelam_phi[step] = lastgaelam = delta + gamma * nonterminal * lastgaelam
    #seg['tdlamret'] = gaelam_phi + seg["vpred"]
